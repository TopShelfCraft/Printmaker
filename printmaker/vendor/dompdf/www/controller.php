...
//include __DIR__."/../dompdf_config.inc.php";
...
switch ($cmd) {
  ...
  case "install-font":
    ...
    foreach($data["error"] as $name => $error) {
      if ($error) {
		$_SESSION['font-error'] = TRUE;
        switch($error) {
          case UPLOAD_ERR_INI_SIZE:
          case UPLOAD_ERR_FORM_SIZE:
            //echo "The uploaded file exceeds the upload_max_filesize directive in php.ini."; break;
			$font_message .= "The uploaded file exceeds the upload_max_filesize directive in php.ini.";
			break;
          case UPLOAD_ERR_PARTIAL: 
            //echo "The uploaded file was only partially uploaded."; break;
			$font_message .= "The uploaded file was only partially uploaded.";
			break;
          case UPLOAD_ERR_NO_FILE: 
			$font_message .= "No file was uploaded!";
            break;
          case UPLOAD_ERR_NO_TMP_DIR: 
            //echo "Missing a temporary folder."; break;
			$font_message .= "Missing a temporary folder.";
			break;
          default: 
            //echo "Unknown error";
			$font_message .= "Unknown error";
        }
        continue;
      }
      ...
      if (!Font_Metrics::register_font($style_arr, $data["tmp_name"][$name])) {
		...
		$font_message .= htmlentities($data["name"][$name])." is not a valid font file";
      }
      else {
        $font_view = htmlentities("$family $weight $style");
        //echo "The <strong>$font_view</strong> font was successfully installed !<br />";
		$font_message .= "The <strong>$font_view</strong> font was successfully installed !<br />";
      }
    }
  break;
}

$_SESSION['font-error'] = $font_message;